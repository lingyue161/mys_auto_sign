...
class MiYoSheSignerApp:
    ...
    def _build_header(self):
        ...
        tk.Label(title_frame, text="v5.0",
    ...
    def _build_control_panel(self, parent):
        ...
        for game_biz, (name, *_) in GAMES.items():
            ...
            tk.Checkbutton(game_section, text=name, variable=var,
                           ...
                           cursor="hand2").pack(anchor="w", padx=12, pady=2)
...
class AboutDialog(tk.Toplevel):
    ...
    def __init__(self, parent):
        ...
        # 内容
        frame = tk.Frame(self, bg=Theme.BG_WHITE, padx=30, pady=20)
        frame.pack(fill="both", expand=True)

        # 标题
        tk.Label(frame, text="米游社自动签到",
                 font=("Microsoft YaHei UI", 18, "bold"),
                 bg=Theme.BG_WHITE, fg=Theme.ACCENT).pack(pady=(0, 2))

        tk.Label(frame, text=self.VERSION,
                 font=("Microsoft YaHei UI", 10),
                 bg=Theme.BG_WHITE, fg=Theme.TEXT_SECOND).pack(pady=(0, 10))

        # 分隔线
        tk.Frame(frame, bg=Theme.BORDER, height=1).pack(fill="x", pady=(0, 12))

        # ── 开发者 & 链接 ──
        info_frame = tk.Frame(frame, bg=Theme.BG_WHITE)
        info_frame.pack(fill="x")

        tk.Label(info_frame, text="开发者：",
                 font=("Microsoft YaHei UI", 11),
                 bg=Theme.BG_WHITE, fg=Theme.TEXT).grid(row=0, column=0, sticky="w", pady=3)
        tk.Label(info_frame, text=self.AUTHOR,
                 font=("Microsoft YaHei UI", 11, "bold"),
                 bg=Theme.BG_WHITE, fg=Theme.ACCENT_DARK).grid(row=0, column=1, sticky="w", padx=(8, 0), pady=3)

        tk.Label(info_frame, text="GitHub：",
                 font=("Microsoft YaHei UI", 11),
                 bg=Theme.BG_WHITE, fg=Theme.TEXT).grid(row=1, column=0, sticky="w", pady=3)
        gh_link = tk.Label(info_frame, text=self.GITHUB,
                           font=("Microsoft YaHei UI", 10),
                           bg=Theme.BG_WHITE, fg=Theme.ACCENT, cursor="hand2", underline=True)
        gh_link.grid(row=1, column=1, sticky="w", padx=(8, 0), pady=3)
        ...
        tk.Label(info_frame, text="抖音主页：",
                 font=("Microsoft YaHei UI", 11),
                 bg=Theme.BG_WHITE, fg=Theme.TEXT).grid(row=2, column=0, sticky="w", pady=3)
        dy_link = tk.Label(info_frame, text="点击访问抖音主页",
                           font=("Microsoft YaHei UI", 10),
                           bg=Theme.BG_WHITE, fg="#FE2C55", cursor="hand2", underline=True)
        dy_link.grid(row=2, column=1, sticky="w", padx=(8, 0), pady=3)
        ...
        # 分隔线
        tk.Frame(frame, bg=Theme.BORDER, height=1).pack(fill="x", pady=12)

        # ── 贡献者 ──
        tk.Label(frame, text="贡献者",
                 font=("Microsoft YaHei UI", 12, "bold"),
                 bg=Theme.BG_WHITE, fg=Theme.TEXT).pack(anchor="w", pady=(0, 8))

        contributors = [
            ("晟曦", "作者", self.PROJECT_DIR / "作者ico.jpg"),
            ("CodeBuddy", "贡献者", None),  # 程序生成
            ("Theresa-0328", "贡献者", self.PROJECT_DIR / "贡献者3.jpg"),
        ]

        contrib_frame = tk.Frame(frame, bg=Theme.BG_WHITE)
        contrib_frame.pack(fill="x")

        for idx, (name, role, avatar_path) in enumerate(contributors):
            col = idx
            cell = tk.Frame(contrib_frame, bg=Theme.BG_SECONDARY, bd=0,
                            highlightbackground=Theme.BORDER, highlightthickness=1)
            cell.grid(row=0, column=col, padx=8, pady=4, sticky="nsew")
            contrib_frame.columnconfigure(col, weight=1)

            # 头像
            avatar_size = 64
            avatar_label = tk.Label(cell, width=avatar_size, height=avatar_size,
                                    bg=Theme.BG_SECONDARY)
            avatar_label.pack(pady=(10, 4))
            ...
            if avatar_path and avatar_path.exists():
                try:
                    ...
                    avatar_label.image = photo  # 防止 GC
                except Exception:
                    self._draw_default_avatar(avatar_label, avatar_size, name)
            else:
                self._draw_default_avatar(avatar_label, avatar_size, name)
            ...
            tk.Label(cell, text=name,
                     font=("Microsoft YaHei UI", 10, "bold"),
                     bg=Theme.BG_SECONDARY, fg=Theme.TEXT).pack(pady=(2, 0))

            # 角色
            role_color = Theme.ACCENT_DARK if role == "作者" else Theme.TEXT_SECOND
            tk.Label(cell, text=role,
                     font=("Microsoft YaHei UI", 9),
                     bg=Theme.BG_SECONDARY, fg=role_color).pack(pady=(0, 8))

        # 免费声明
        free_frame = tk.Frame(frame, bg="#FFF3E0", bd=0,
                              highlightbackground="#FFB74D", highlightthickness=1)
        free_frame.pack(fill="x", pady=(8, 4))
        tk.Label(free_frame, text="⚠ 此软件完全免费，如果你是花钱买的请尽快退款！",
                 font=("Microsoft YaHei UI", 10, "bold"),
                 bg="#FFF3E0", fg="#E65100",
        ...
        tk.Label(frame, text="本工具仅供学习交流使用，与米哈游官方无关。",
                 ...
                 bg=Theme.BG_WHITE, fg=Theme.TEXT_SECOND,
                 wraplength=380).pack(side="bottom", pady=(8, 0))

        # 确认按钮
        btn = tk.Button(frame, text="我已了解，进入主界面", command=self._close,
                        font=("Microsoft YaHei UI", 11, "bold"),
